...
class PembayaranKereta extends CI_Controller
{
    public $_api_context;
    ...
    {
        parent::__construct();
        $this->load->model('users/pembayaranKeretaModel', 'paypal');
    ...
    public function pesanPembayaran()
    {
        ...
        if($typePembayaran == 'manual')
        {
	        ...
	        else
	        {
				...
				redirect('users/pembayaran/uploadBuktiPembayaran/'.$exePemesan,'refresh');
	        }
        }
...
// information
        $item1["name"] 			= 'Tiket Kereta';
        $item1["sku"] 			= 'TIKK-0000';  // Similar to `item_number` in Classic API
        $item1["description"] 	= 'Tiket Kereta Dari '.$kotaAsal.' Ke '.$kotaTujuan;
        $item1["currency"] 		= 'USD';
...
// payment approval/ cancellation.
        ...
        $redirectUrls->setReturnUrl($baseUrl."users/pembayaranKereta/getPaymentStatus")
            ->setCancelUrl($baseUrl."users/pembayaranKereta/getPaymentStatus");
...
// the above types and intent set to sale 'sale'
    ...
    public function getPaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            $data['getDataKererta']         = $this->paypal->getPerjalanan()->result_array();
