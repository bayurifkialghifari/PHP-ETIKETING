...
class GenerateModel extends CI_Model {
	...
	{
		$this->db->select('a.*, b.*, c.*, d.*, e.*, f.*, g.kota_nama as kota_asal, g.kota_id as kota_asal_id, h.kota_nama as kota_tujuan, h.kota_id as kota_tujuan_id, i.band_nama as band_asal, i.band_id as band_asal_id, j.band_nama as band_tujuan, j.band_id as band_tujuan_id, 
				...
				FROM tiket_pesawat_detail j 
					...
					WHERE j.tipd_tikp_id = a.tikp_id 
				
				LIMIT 1) as tikp_harga_usd,
			');

		$this->db->from('tiket_pesawat a');
		// $this->db->join('tiket_pesawat_detail k', 'a.tikp_id = k.tipd_tikp_id', 'left');
		...
		$this->db->where("(b.jadp_kode LIKE '%".$cari."%' or e.mask_nama LIKE '%".$cari."%' or d.pesa_nama LIKE '%".$cari."%' or g.kota_nama LIKE '%".$cari."%' or h.kota_nama LIKE '%".$cari."%' or i.band_nama LIKE '%".$cari."%' or j.band_nama LIKE '%".$cari."%' or b.jadp_tanggal_berangkat LIKE '%".$cari."%' or b.jadp_tanggal_berangkat_sampai LIKE '%".$cari."%' ) ");
